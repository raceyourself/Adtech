 ...
 * Annex B file.
 */
public class GIH264TrackImpl extends AbstractTrack {
     ...
     */
    public GIH264TrackImpl(DataSource fc, String lang, long timescale, int frametick) throws IOException {
        this.lang = lang;
    ...
    public GIH264TrackImpl(DataSource fc, String lang) throws IOException {
        this.lang = lang;
        this.dataSource = fc;
        parse(new LookAhead(dataSource));
    }

    public GIH264TrackImpl(DataSource fc) throws IOException {
        this.dataSource = fc;
        parse(new LookAhead(dataSource));
    }
    ...
    class LookAhead {
        ...
        boolean nextEqualsTcpMarker() throws IOException {
            if (bufferStartPos + inBufferPos == dataSource.size()) {
                throw new EOFException();
            }
            return (buffer.get(inBufferPos) == '$');
        }

        void parseTcpHeaderMarkStartAndJumpToEnd() throws IOException {
            if (buffer.limit() - inBufferPos >= 4) {
                byte marker = buffer.get(inBufferPos);
                if (marker != '$') throw new RuntimeException("buffer not at TCP marker");
                byte channel = buffer.get(inBufferPos+1);
                byte len1 = buffer.get(inBufferPos+2);
                byte len2 = buffer.get(inBufferPos+3);
                int length = (len1 << 8) + (len2 & 0xff);
                ...
                // Jump to end
                inBufferPos += length;
            } else throw new RuntimeException("buffer out of bounds");
        }
        ...
        boolean nextThreeEquals001() throws IOException {
            if (buffer.limit() - inBufferPos >= 3) {
                ...
                System.err.println(GIH264TrackImpl.this.samples.size());
    ...
    private ByteBuffer findNextSample(LookAhead la) throws IOException {
        try {
            ...
            //Log.i("Imp", "Sample start at " + la.start);

            while (!la.nextThreeEquals000or001orEof()) {
                la.discardByte();
            }
            //Log.i("Imp", "Sample end at " + la.inBufferPos);
            return la.getSample();
    ...
    private boolean readSamples(LookAhead la) throws IOException {
        ...
        while ((sample = findNextSample(la)) != null) {
            Log.e("H264", "Sample len: " + sample.limit());
